...
//        response.getWriter().close();
    ...
    protected void validateAula(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            SediFormazione p = e.getEm().find(SediFormazione.class, Long.valueOf(getRequestValue(request, "id")));
            switch (status) {
                case "OK" -> p.setStato("A");
                case "OK1" -> p.setStato("A1");
                case "KO" -> p.setStato("R");
                default -> p.setStato("DV");
            }
            e.merge(p);
            e.commit();
            resp.addProperty("result", true);
...
//        }
        ...
        try {
            ...
            if (p.getStato().getId().equals("DC")) {
                ...
                if (!neets.equalsIgnoreCase("[]") && !neets.equalsIgnoreCase("")) {
                    ...
                    for (int i = 0; i < jsonarray.length(); i++) {
                        ...
                        temp.setStatopartecipazione(e.getEm().find(StatoPartecipazione.class, "09"));
            ...
            if (!rigettaPrg) {
                ...
                e.persist(new Storico_Prg("Rigettato automaticamente: numero minimo di partecipanti (" + min_allievi + ") non raggiunto",
                        new Date(), p, p.getStato()));//storico progetto
                rigettaPrg_msg = "Numero minimo di partecipanti (" + min_allievi + ") per avviare il Progetto Formativo non raggiunto";
            }
            if (check) {
                if (!rigettaPrg) {
    ...
    protected void annullaPrg(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            p.getAllievi().forEach(al1 -> {
                al1.setStatopartecipazione(e.getEm().find(StatoPartecipazione.class, "11"));
                e.merge(al1);
            });
            e.merge(p);
            e.commit();
...
//                File content = new File("F:\\mnt\\mcn\\BECONSULTING260720211205764.M2_pdfA.pdf");//R^TEST
                ...
                if (content.exists()) {
                    ...
                    testo = StringUtils.replace(testo, "@soggetto", pf.getSoggetto().getRagionesociale());
    ...
    protected void checklistFinale(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ProgettiFormativi pf = e.getEm().find(ProgettiFormativi.class, Long.valueOf(request.getParameter("pf")));
            ...
            switch (step) {
                ...
                case "2" -> {
                    cfinal.setTot_contributo_indennita_frequenza_fa(parseDouble(request.getParameter("fa_total")));
                    cfinal.setTot_contributo_fb(parseDouble(request.getParameter("fb_total")));
                    cfinal.setTot_docenza_fa(parseDouble(request.getParameter("dc_total")));
                    cfinal.setTot_massimo_ammissibile(parseDouble(request.getParameter("maxammissibile")));
                    cfinal.setCondizionalita_30perc(parseDouble(request.getParameter("cond30")));
                    cfinal.setVcr_70perc(parseDouble(request.getParameter("vcr70")));
                ...
                case "3" -> {
                    ...
                    cfinal.setTot_contributo_ammesso(parseDouble(request.getParameter("tot_contributo_ammesso")));
